...
def s_eval(expr, namespace):
    try:
        tree = ast.parse(expr, mode="eval")
        se = SEval(allowed_names=namespace.keys())
        se.visit(tree)
...
def main():
    ...
    parser.add_argument("--filter", required=True, help="filter expression")
    ...
    for _, row in df.iterrows():
        ...
        try:
            if s_eval(args.filter, namespace):
